...
export const createComment = async (req: Request<{ answerId: string }, {}, CreateCommentDTO>, res: Response) => {
  try {
    ...
    const comment = new Comment({ Text, PreviousComment, Author, Rating: createdRating._id });
...
export const getAllComments = async (req: Request<{ answerId: string }, {}, {}>, res: Response) => {
  try {
    // Get the answer by its ID
    const answer = await Answer.findById(req.params.answerId);
...
export const getComment = async (req: Request<{ commentId: string }, {}, {}>, res: Response) => {
  try {
...
export const updateComment = async (req: Request<{ commentId: string }, {}, CreateCommentDTO>, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
