...
export const createAnswer = async (req: Request<{ questionId: string }, {}, CreateAnswerDTO>, res: Response) => {
  try {
...
export const getAllAnswers = async (req: Request<{ questionId: string }, {}, {}>, res: Response) => {
  try {
    // Get the question by its ID
    const question = await Question.findById(req.params.questionId);
...
export const getAnswer = async (req: Request<{ answerId: string }, {}, {}>, res: Response) => {
  try {
...
export const updateAnswer = async (req: Request<{ answerId: string }, {}, CreateAnswerDTO>, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
