...
export const getAllCoursesInUniversity = async (req: Request, res: Response) => {
  try {
    // get the university by its ID
    const university = await University.findById(req.params.universityId);
...
export const updateCourse = async (req: Request<{ id: string }, {}, CourseDTO>, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    ...
    const updatedCourse = await Course.findByIdAndUpdate(req.params.id, { Name }, { new: true });
