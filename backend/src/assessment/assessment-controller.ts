...
export const createAssessment = async (req: Request<{ courseId: string }, {}, CreateAssessmentDTO>, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    }
    const { AssessmentType, Year, Semester } = req.body; // assuming request body contains assessment data
    ...
    const existingAssessment = await Assessment.findOne({ AssessmentType, Year, Semester, Course: req.params.courseId });

    if (existingAssessment) {
      return res.status(400).json({ error: "Assessment with the same name already exists in this course" });
    }

    // create a new assessment instance
    const assessment = new Assessment({ AssessmentType, Year, Semester, Course: req.params.courseId });
...
export const getAllAssessmentsInCourse = async (req: Request<{ courseId: string }, {}, {}>, res: Response) => {
  try {
    // fetch all assessments from the database
...
export const getAssessment = async (req: Request<{ id: string }, {}, {}>, res: Response) => {
  try {
...
export const updateAssessment = async (req: Request<{ id: string }, {}, CreateAssessmentDTO>, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
    ...
    const { AssessmentType, Year, Semester } = req.body;
...
export const deleteAssessment = async (req: Request<{ id: string }, {}, {}>, res: Response) => {
  try {
